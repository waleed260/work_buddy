...
async def test_remote_work_buddy():
    ...
    try:
        ...
        print(
            "   Note: Chat requires a valid API key (OPENAI_API_KEY or GEMINI_API_KEY)"
        )
    ...
    try:
        response = await buddy.chat(
            "Schedule my day. I have 3 priority tasks: project review, team meeting, and code review."
        )
        print(f"\n🤖 Agent Response:\n{response}")
    except Exception as e:
