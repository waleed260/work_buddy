...
def _get_daily_standup() -> str:
    ...
    for t in completed[-5:]:
        standup += f"  ✅ {t.title}\n"
    if not completed:
        standup += "  🌊 Nothing completed yet—let's catch a wave!\n"

    standup += "\n**In Progress:**\n"
    if not pending:
        standup += "  ✨ No pending tasks! You're a productivity ninja! 🥷\n"
    else:
...
def _get_email_drafts() -> str:
    ...
    if not _email_drafts:
        return "📧 No email drafts yet. Ready to write something great? ✨"
...
def _get_slack_messages() -> str:
    ...
    if not _slack_messages:
        return "💬 No Slack messages drafted. Your team is waiting for your brilliance! 🚀"
...
def _extract_action_items(transcript: str) -> str:
    ...
    if not action_items:
        return "✨ No action items found. A smooth meeting indeed! 🌊"
