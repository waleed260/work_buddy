...
def _get_daily_standup() -> str:
    ...
    for t in completed[-5:]:
        standup += f"  ✅ {t.title}\n"
    if not completed:
        standup += "  ✨ All caught up! (none yet)\n"

    standup += "\n**In Progress:**\n"
    if not pending:
        standup += "  🌈 No pending tasks! (none)\n"
    else:
...
def _get_email_drafts() -> str:
    ...
    if not _email_drafts:
        return "📧 Your outbox is clear! No email drafts waiting. ✨"
...
def _get_slack_messages() -> str:
    ...
    if not _slack_messages:
        return "💬 No Slack messages in the works. Your team is up to date! 🚀"
