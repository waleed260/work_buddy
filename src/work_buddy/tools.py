...
def _get_daily_standup() -> str:
    ...
    standup += "\n**In Progress:**\n"
    if not pending:
        standup += "  🌊 All tasks caught up! Time for a break? 🧘\n"
    else:
...
def _get_email_drafts() -> str:
    ...
    if not _email_drafts:
        return "📧 ✨ No email drafts yet. Ready to compose something great? 🚀"
...
def _get_slack_messages() -> str:
    ...
    if not _slack_messages:
        return "💬 🌊 No Slack messages drafted. Your team is waiting for an update! ✨"
