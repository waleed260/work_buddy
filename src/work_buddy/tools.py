...
def _get_daily_standup() -> str:
    ...
    for t in completed[-5:]:
        standup += f"  ✅ {t.title}\n"
    if not completed:
        standup += "  ✨ Nothing completed yet. Let's get started! 🚀\n"

    standup += "\n**In Progress:**\n"
    if not pending:
        standup += "  🌊 No tasks in progress. Ready for a new challenge? 🥷\n"
    else:
...
def _get_email_drafts() -> str:
    ...
    if not _email_drafts:
        return "📧 No email drafts yet. Your inbox is waiting for some magic! ✨"
...
def _get_slack_messages() -> str:
    ...
    if not _slack_messages:
        return "💬 No Slack messages drafted. Ready to connect with the team? 🌊"
...
def _extract_action_items(transcript: str) -> str:
    ...
    if not action_items:
        return "✨ No action items found. Everything is on track! 🚀"
