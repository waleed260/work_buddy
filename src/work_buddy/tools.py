...
def _get_daily_standup() -> str:
    ...
    for t in completed[-5:]:
        standup += f"  ✅ {t.title}\n"
    if not completed:
        standup += "  (none yet - time to make a splash! 🌊)\n"

    standup += "\n**In Progress:**\n"
    if not pending:
        standup += "  (none - you're a productivity ninja! 🥷)\n"
    else:
...
def _get_email_drafts() -> str:
    ...
    if not _email_drafts:
        return "📧 No email drafts yet. Your inbox is breathing easy! ✨"
...
def _get_slack_messages() -> str:
    ...
    if not _slack_messages:
        return "💬 No Slack messages drafted. Team communication is all clear! 🚀"
