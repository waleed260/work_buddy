...
def _get_daily_standup() -> str:
    ...
    for t in completed[-5:]:
        standup += f"  ✅ {t.title}\n"
    if not completed:
        standup += "  ✨ No completed tasks yet. Let's get started! 🚀\n"

    standup += "\n**In Progress:**\n"
    if not pending:
        standup += "  🌊 No tasks in progress. High five! 🙌\n"
    else:
...
def _get_email_drafts() -> str:
    ...
    if not _email_drafts:
        return "📧 Your draft folder is empty. Time to reach out? 🌊"
...
def _get_slack_messages() -> str:
    ...
    if not _slack_messages:
        return "💬 No Slack drafts yet. Keeping it quiet? 🥷"
